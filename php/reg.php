<?php

// 通过include函数或者require函数加载数据库连接的文件。
include 'connect.php';

// isset():判断括号里面的值是否存在。

// 1.用户名重名检测
if(isset($_POST['checkname'])){
  $checkname = $_POST['checkname'];
  $result = $conn->query("SELECT * FROM haierreg WHERE username='$checkname'");
  //$result->fetch_assoc():逐条获取$result里面每一条记录,返回值是一个数组。
  if($result->fetch_assoc()){//满足条件，存在值，说明此用户名不能被注册。
    echo 'true';
  }else{  
    echo 'false';
  }
}

// 2.将前端传入的值放入数据库
// $_POST['submit']:获取提交注册按钮的值
if(isset($_POST['submit'])){//存在，说明前端已经点击了提交注册的按钮。
  $phone=$_POST['phone'];//获取用户名
  $user=$_POST['username'];//获取用户名
  $pass=sha1($_POST['password']);//获取用密码

  //将前端提交的数据存入数据库 - sql语句
  $conn->query("insert haierreg values(null,'$phone','$user','$pass')");

  //后端跳转
  header('location:http://10.31.165.55/haier/src/login.html');

  //后端跳转前端，前端跳转后端都需要采用绝对路径，但是如果是前端跳转前端，后端跳转后端可以使用相对路径。
}